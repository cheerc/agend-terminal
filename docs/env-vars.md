...
| `AGENTIC_GIT_REAL_GIT`<br>`AGEND_REAL_GIT` (legacy) | Escape hatch holding the path to the real git binary so the shim execs git without recursing. Daemon injects it at agent spawn. | Shim: unset → falls back to literal `"git"` then PATH-exclude resolution. Daemon: only injected when not already set. | Absolute path to a git executable; accepted only if it exists. | `vendor/agentic-git/crates/agentic-git/src/exec.rs:87` (read); injected at `src/agent/mod.rs:835` | ⚠️ Correctness-sensitive: wrong/missing value risks recursive-spawn storm (#1504). The `AGEND_*` spelling is accepted as a legacy fallback. |
| `AGENTIC_GIT_HOME`<br>`AGEND_HOME` (legacy) | The agentic-git shim's own home: the directory whose `bin/` the shim excludes from the PATH it hands the real git, and where it writes its audit lines. Daemon pins it to the daemon home at agent spawn (#1504 family). | Shim: unset → falls back to `AGEND_HOME`. Daemon: injected whenever a home is known. | Absolute directory path, equal to the injected `AGEND_HOME`. | `vendor/agentic-git/crates/agentic-git/src/exec.rs:110` (read); injected at `src/agent/mod.rs:936` | ⚠️ In `SENSITIVE_ENV_KEYS`: a fleet.yaml / template env cannot redirect it. Without the pin, tests that scope `AGEND_HOME` (`ScopedAgendHome`) and spawn `git` make the shim resolve to itself and trip its recursion guard (~40 deterministic failures in agent shells; CI has no shim on PATH). |
